...
export default function Edit({ employmentType }: Props) {
    const { data, setData, put, processing, errors } = useForm({
        ...
        display_order: employmentType.display_order,
        is_active: employmentType.is_active,
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        put(`/admin/employment-types/${employmentType.id}`);
    };
        ...
        <AppLayout>
            ...
            <div className="p-6">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-bold">雇用形態編集</h1>
                ...
                <form onSubmit={submit} className="mt-6 space-y-6 rounded-lg border p-6">
                    <div>
                        <label className="mb-2 block text-sm font-medium">雇用形態名</label>
                        <input
                            type="text"
                            value={data.name}
