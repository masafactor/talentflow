...
export default function Create({ evaluation, employees }: Props) {
    ...
    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        post(`/admin/evaluations/${evaluation.id}/feedbacks`);
    };

    return (
        <AppLayout>
            <Head title="フィードバック記録登録" />

            <div className="p-6">
                <div className="flex items-center justify-between">
                    <h1 className="text-2xl font-bold">フィードバック記録登録</h1>
                ...
                <form onSubmit={submit} className="mt-6 space-y-6 rounded-lg border p-6">
                    ...
                    <div>
                        ...
                        {errors.feedbacker_employee_id && <div className="mt-1 text-sm text-red-600">{errors.feedbacker_employee_id}</div>}
                    </div>
